...
<xsl:template match="item" mode="categorylist_main2">
	...
	<xsl:template match="item" mode="categorylist_main_brend">
		...
		<div class="cat_item">
			...
			<h4><a href="{@link}"><xsl:if test="$cat"><xsl:attribute name="href"><xsl:value-of select="@link"/>?fields_filter[brend]=<xsl:value-of select="$cat" /></xsl:attribute></xsl:if><xsl:value-of select="."/></a></h4><span></span>
		</div>
	</xsl:template>
...
<!--					плашечка скидки на картинке товара в каталоге-->
			...
			<div class="good_bookmopt">
				<ul class="good_bookmarks">
					...
					<xsl:if test="page/properties/group/property[@name='video']/value/page">
						<li><a name="tab5" href="javascript://">Видео</a>
						</li>
					</xsl:if>
	...
	<xsl:template match="items/item" mode="search_select1">
...
<script type="text/javascript">
    $( document ).ready(function() {
        ga('ec:addProduct', {
           'id': '<xsl:value-of select="page/properties/group/property[@name='artikul']/value" />',
           'name': '<xsl:value-of select="page/properties/group/property[@name='nazvanie']/value" />',
           'category': '<xsl:value-of select="document(concat('upage://', parents/page[last()]/@id))//property[@name='h1']/value" />',
           'brand': '<xsl:value-of select="page/properties/group/property[@name='brend']/value/item/@name" />'
        });
        ...
        $('div.buy_price').on( "click", "input", function() {
             ga('ec:addProduct', {
                'id': '<xsl:value-of select="page/properties/group/property[@name='artikul']/value" />',
                'name': '<xsl:value-of select="page/properties/group/property[@name='nazvanie']/value" />',
                'category': '<xsl:value-of select="document(concat('upage://', parents/page[last()]/@id))//property[@name='h1']/value" />',
                'brand': '<xsl:value-of select="page/properties/group/property[@name='brend']/value/item/@name" />',
                'price': '<xsl:value-of select="page/properties/group/property[@name='price']/value" />',
                'quantity': '1'
              });
              ga('ec:setAction', 'add');
...
<!--                            </a>-->
        ...
        <article>
            ...
            <div class="good_bookmopt">
                <ul class="good_bookmarks">
                    <li>
                        <a name="tab1" href="javascript://">Описание</a>
                    </li>
                    <li>
                        <a name="tab2" href="javascript://">Характеристики</a>
                    </li>
                    <xsl:if test="page/properties/group/property[@name='video']/value/page">
                    <li>
                        <a name="tab5" href="javascript://">Видео</a>
                    </li>
                    </xsl:if>
                    
                    <li>
                        <a name="tab3" href="javascript://">Загрузки</a>
                    </li>                    
                    <li>
...
<!--    шаблоны для ации и распродаж-->
    ...
    <xsl:template match="udata" mode="brand">
        ...
        <xsl:value-of select="object/properties/group/property[@name='strana']/value/item/@name" />
    </xsl:template>
    ...
    <xsl:template match="property[@type='wysiwyg' or @type='img_file' or @name='nazvanie' or @name='kratkoe_opisanie']" mode="properties">
...
<!-- <xsl:value-of select="$typeid" /> -->
        <div class="back_filterslid">
            ...
            <p>
                ...
                <div class="slider-range" id="{@name}"></div>
                <xsl:choose>
                    <xsl:when test="not (value_to) or value_to=0">
                        от
                        <input id="min_{@name}" placeholder="0" class="min_val" name="fields_filter[{@name}][0]" size="10" type="text" title="не менее" />
                    </xsl:when>
                    <xsl:otherwise>
