...
$(document).ready(function () {
	...
	var validationFRM = function (nameTab) {
		//	function validationFRM(nameTab) {

		$('#payment_choose #' + nameTab).each(function () {



			// Объявляем переменные (форма и кнопка отправки)
			var formfield = $(this),
				btn = formfield.parent().find('.checkout_btnxt');


			// Добавляем каждому проверяемому полю, указание что поле пустое
//			formfield.find('.ntSaveForms').addClass('empty_field');
			...
			var dianbl = function () {
				...
				var sizeEmpty = formfield.find('.empty_field').size();
				...
				if (sizeEmpty > 0) {
					if (btn.hasClass('disabled')) {
						...
						kjhgkjhg = setInterval(dianbl, 200);
					}
				} else {
					btn.removeClass('disabled');
					clearInterval(kjhgkjhg);
			...
			btn.click(function () {
				...
				if ($(this).hasClass('disabled')) {
					...
					//					имитируем клик на последнюю закладку
					$("section.cartpage_sec article ul li a[name='tab" + tabIdForButton + "']").trigger('click');
	...
	$("a[name='tab2']").click(function () {
		...
		if ($("#radio34011").prop("checked") == false) {
	...
	if ($("ul.tabsch li").hasClass("active_chbtn")) {
		validationFRM("tab1");
	...
	var handlervalidationFRM = function () {
		validationFRM("tab2");
		console.log('444444444444444');
	};

	$("#radio339").bind('click', handlervalidationFRM);
	$("#radio12808").bind('click', handlervalidationFRM);

	$("#radio34011").click(function () {
		$("#radio339").unbind('click');
		$("#radio12808").unbind('click');
	});
	...
	//	$("section.cartpage_sec article ul li a[name='tab" + tabIdForButton + "']").click(function () {
	//
	//
	//
	//		if ($("#radio34011").prop("checked") == false) {
	//
	//			console.debug("ID bkmrk:" + tabIdForButton);
	//			validationFRM("tab2");
	//
	//
	//		}
